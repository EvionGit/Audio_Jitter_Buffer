...
namespace jbuf
{
	...
	JitterBuffer::JitterBuffer(uint32_t buffering_time_ms, uint32_t payload_ms_per_packet, uint32_t max_jitter_ms)
	{
	...
	int JitterBuffer::push(AudioPacket& packet, Ttimepoint arrived_time)
	{
		...
		if (current_buffer_size_ms > max_jitter_ms) // circular overflow
		{
